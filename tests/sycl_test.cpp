...
TEST_CASE("test_example1")
{
  using namespace sycl;
  std::vector<float> data(1024, 0);

  {
    queue myQueue;
    buffer<float, 1> resultBuf{ data.data(), range<1>{1024}};
    myQueue.submit([&](handler& cgh)
    {
      auto writeResult = resultBuf.get_access<access::mode::write>(cgh);
      cgh.parallel_for(range<1>{1024}, [=](id<1> idx)
  ...
  for (int i = 0; i < 1024; i++)
  {
    std::cout << "vdata[" << i << "] = " << data[i] << std::endl;
  }
}

TEST_CASE("test_example2")
{
