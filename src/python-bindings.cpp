...
PYBIND11_MODULE(nervalib, m)
{
  ...
  py::class_<multilayer_perceptron, std::shared_ptr<multilayer_perceptron>>(m, "MLP")
    ...
    .def("export_weights_and_bias", [](const multilayer_perceptron& M, const std::string& filename) { export_weights_and_bias_to_npz(M, filename); })
    .def("import_weights_and_bias", [](multilayer_perceptron& M, const std::string& filename) { import_weights_and_bias_from_npz(M, filename); })
    ;
