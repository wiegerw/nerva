...
void check_accept_count(const EigenMatrix& A, long expected, Accept accept=Accept())
{
  long N = A.rows() * A.cols();
  const scalar* data = A.data();
  ...
  if (count != expected)
  {
    throw std::runtime_error("check_accept_count failed: expected = " + std::to_string(count) + " found = " + std::to_string(count));
  }
}
...
// Remove the smallest k elements (in absolute size) and randomly add new elements
// for them. If the k-th smallest element is tied, all copies are removed, even if
// that causes more than k elements to be removed.
template <typename EigenMatrix, typename Scalar = scalar>
