...
#ifndef NERVA_LOGGER_H
#define NERVA_LOGGER_H
...
typedef logger nerva_logger;

/// \brief Unless otherwise specified, we compile away all debug messages that have
/// a log level greater than NERVAMaxLogLevel.
constexpr log_level_t NERVAMaxLogLevel = nerva::log::debug;

/// \returns True whenever the logging for the given level is enabled.
constexpr bool NERVA_LOG_ENABLED(const log_level_t level)
{
  return level <= NERVAMaxLogLevel && level <= nerva_logger::default_reporting_level();
}

/// \returns True whenever the logging for the given level and hint is enabled.
constexpr bool NERVA_LOG_ENABLED(const log_level_t level, const std::string& hint)
{
  return level <= NERVAMaxLogLevel && level <= nerva_logger::get_reporting_level(hint);
}

} // namespace nerva::log

/// \brief NERVA_LOG(level) or NERVA_LOG(level, hint) provide the stream used to log.
/// \details Uses variadic macros to allow NERVA_LOG(level) as well as NERVA_LOG(level, "hint").
#define NERVA_LOG(...) if (nerva::log::NERVA_LOG_ENABLED(__VA_ARGS__)) nerva::log::nerva_logger().get(__VA_ARGS__)

#endif // NERVA_LOGGER_H
