...
def minibatch_gradient_descent_with_regrow(M, dataset, loss, learning_rate, epochs, batch_size, shuffle=True, statistics=True, zeta=0.3, regrow_weights=Weights.Zero, rng=RandomNumberGenerator(1234567)):
    ...
    for epoch in range(epochs):
        if epoch > 0:
            M.regrow(zeta, regrow_weights, rng)
...
def train_sparse_model_with_regrow(dataset):
    ...
    model = create_sparse_model(sparsity)
    M = model.compile(input_size, batch_size, rng)
    minibatch_gradient_descent_with_regrow(M, dataset, loss, learning_rate_scheduler, epochs=10, batch_size=100, shuffle=True, statistics=True, zeta=0.3, regrow_weights=Weights.Zero, rng=rng)
    print('')
