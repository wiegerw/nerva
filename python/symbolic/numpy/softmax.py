...
def softmax_colwise_jacobian(x: np.ndarray) -> np.ndarray:
...
def log_softmax_colwise_jacobian(x: np.ndarray) -> np.ndarray:
    assert is_column_vector(x)
    D, N = x.shape
...
def softmax_rowwise_jacobian(x: np.ndarray) -> np.ndarray:
...
def log_softmax_rowwise_jacobian(x: np.ndarray) -> np.ndarray:
    assert is_row_vector(x)
    N, D = x.shape
